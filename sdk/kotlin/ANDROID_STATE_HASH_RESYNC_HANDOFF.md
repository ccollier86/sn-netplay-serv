...
The relay now treats the first confirmed exact-frame state-hash mismatch as a
real resync trigger. Nearby-frame matches are still included in the mismatch
payload for diagnostics, but they do not suppress repair. The goal is to avoid
letting visible drift continue after the relay knows the same emulated frame
does not match.
...
chunks and snapshot completion messages include `roomEpoch` and `sessionEpoch`
at the message level, and they must match the current runtime epoch exactly.
Android should still avoid submitting ignored runtime payloads to the emulator
runner; apply the message first, or call
`roomStateMachine.isRuntimeMessageCurrent(message)` before touching emulator
state.

Snapshot payloads also carry exact repair identity:

```kotlin
SnapshotChunk(
    snapshotId = "...",
    repairFrame = mismatch.repairFrame,
    index = chunkIndex,
    bytes = bytes,
)

SnapshotManifest(
    snapshotId = sameSnapshotId,
    repairFrame = sameRepairFrame,
    totalBytes = totalBytes,
    sha256 = sha256,
)
```

Reject/reset a received transfer if `snapshotId` or `repairFrame` changes
mid-transfer. Reject the manifest if it does not match the received transfer.
...
`ServerMessage.StartSession.startFrame` is the canonical frame that the runtime
must restart from after repair. For active state-hash repair this should match
the snapshot `repairFrame`; do not restart from frame `0`.
