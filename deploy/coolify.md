...
The response must identify the deployed `latest` image and embedded source revision:

```json
{
  "status": "ok",
  "buildSha": "<complete-commit-sha>",
  "imageIdentity": "ghcr.io/ccollier86/sb-netplay-serv:latest",
  "version": "0.1.0",
...
Compare `buildSha` with the commit used by the local Docker build before live
client acceptance. A healthy response from a different SHA is not a successful
rollout.
