...
//! with different ROMs, cores, settings, cheats, or system data.

use serde::{Deserialize, Serialize};

/// Netplay-relevant compatibility fingerprint for one Desktop client.
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct CompatibilityFingerprint {
    /// ShadowBoy Desktop application version.
...
impl CompatibilityFingerprint {
    ...
    pub fn first_mismatch(&self, other: &Self) -> Option<CompatibilityMismatch> {
        ...
        if self.core_id != other.core_id || self.core_build != other.core_build {
...
pub enum CompatibilityMismatch {
    ...
    System,
    /// Clients selected different cores or core builds.
    Core,
