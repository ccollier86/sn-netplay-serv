...
fn recovery_stays_on_old_epoch_until_exact_host_snapshot_is_pinned() {
    ...
    assert_eq!(committed.pinned_snapshot, Some(manifest));
...
fn third_recovery_attempt_inside_one_minute_closes_the_room() {
    ...
    for attempt in 0..2 {
        assert!(matches!(
            fixture.room.begin_v5_state_recovery(
                mismatch(60 + attempt, char::from(b'a' + attempt as u8)),
    ...
    assert!(matches!(
        fixture
            .room
            .begin_v5_state_recovery(mismatch(62, 'c'), now + Duration::from_secs(2),),
...
fn recovery_attempt_budget_uses_a_rolling_window() {
    ...
    assert!(matches!(
        fixture.room.begin_v5_state_recovery(mismatch(60, 'a'), now),
        Ok(StateRecoveryStartOutcome::Preparing(_))
    ));
    fixture.room.finish_v5_state_recovery();
    fixture.room.status = RoomStatus::Playing;
    ...
    assert!(matches!(
        fixture
            .room
            .begin_v5_state_recovery(mismatch(120, 'b'), now + Duration::from_secs(60),),
        Ok(StateRecoveryStartOutcome::Preparing(_))
    ));
    assert_eq!(fixture.room.status(), RoomStatus::RepairingState);
}
...
fn begin_recovery(
    ...
    fixture
        .room
        .accept_state_hash(fixture.host_control, state_hash(60, 'a'), now)
        .expect("host digest");
    match fixture
        .room
        .accept_state_hash(fixture.guest_control, state_hash(60, 'b'), now)
        .expect("guest digest")
    {
